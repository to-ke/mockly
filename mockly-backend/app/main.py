...
async def get_live_transcription():
    ...
    try:
        ...
        if not LIVE_TRANSCRIPTION_PATH:
            raise HTTPException(status_code=404, detail="Live transcription not enabled")
        
        file_path = Path(LIVE_TRANSCRIPTION_PATH)
        if not file_path.is_absolute():
            # Relative path, resolve from project root
            file_path = PROJECT_ROOT / file_path
        
        if not file_path.exists():
            raise HTTPException(status_code=404, detail="Transcription file not yet available")
